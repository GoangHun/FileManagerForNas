...
### 6. AI 기반 스마트 검색 백엔드 구현

사용자가 자연어로 파일 내용을 검색할 수 있는 AI 기반 스마트 검색 기능을 백엔드에 구현했습니다.

-   **라이브러리 추가:** `requirements.txt`에 `chromadb` (벡터 데이터베이스)와 `sentence-transformers` (임베딩 모델)를 추가했습니다.
-   **`SearchService` 구현:** `backend/search_service.py` 파일을 생성하고, `SearchService` 클래스를 구현했습니다. 이 클래스는 ChromaDB 클라이언트 초기화, 텍스트 파일 내용 임베딩 및 인덱싱, 그리고 쿼리 기반 유사도 검색 기능을 제공합니다.
-   **`main.py` 통합:**
    -   애플리케이션 시작 시 `SearchService` 인스턴스를 초기화하도록 `startup_event`를 수정했습니다.
    -   `/api/search` 엔드포인트를 추가하여 사용자 쿼리를 받아 `SearchService`를 통해 유사한 파일을 검색하고 결과를 반환합니다.
    -   `/api/index_files` 엔드포인트를 추가하여 지정된 디렉토리의 텍스트 파일을 스캔하고 내용을 ChromaDB에 인덱싱하는 기능을 제공합니다. (현재는 로컬 파일 시스템만 지원)
-   **기능 테스트:** `/api/index_files`를 통해 `doc` 폴더를 성공적으로 인덱싱하고, `/api/search`를 통해 "백엔드 관련 문서"와 같은 자연어 쿼리로 관련 문서를 성공적으로 검색하는 것을 확인했습니다.

## 다음 계획
-   **AI 기반 스마트 검색 프론트엔드 통합:** 백엔드에 구현된 검색 기능을 프론트엔드 UI에 통합하여 사용자가 직접 검색하고 결과를 확인할 수 있도록 구현.
